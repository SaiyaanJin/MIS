...
def ExRlogin():
    try:
        resp = requests.post(
            "https://logbook.erldc.in:55005/api/Account/Login",
...
def VoltageReport2(date_val):
    ...
    for item in date_val:
        ...
        dateitem = item.strftime("%d-%m-%Y")

        res1 = requests.get(
            'https://report.erldc.in/posoco_api/api/PSP/GetPSPVoltageProfile_400kvByDate/'+dateitem)
        response1 = json.loads(res1.text)

        for item1 in response1:
            if item1['STATION_NAME']=='Godda_TPS - 400KV':
                response1.remove(item1)

        try:
            response1 = sorted(response1, key=lambda x: x['STATION_NAME'])
            
            response1= list(divide_chunks(response1, 3))
            
            dummy_dict={
                'STATION_NAME':'Dummy',
            }

            if len(response1[-1])!=3:
                if len(response1[-1])==2:
                    response1[-1].append(dummy_dict)
                else:
                    response1[-1].append(dummy_dict)
                    response1[-1].append(dummy_dict)

            for ind2 in range(len(response1)):  
                for ind3 in range(len(response1[ind2])):
                    if ind3==0:
                        if response1[ind2][ind3]['STATION_NAME']== 'Dummy':
                            temp_obj= {"Date_Val": dateitem,
                                    "Name":'',
                                    "Max": ['','x'!='x'],
                                    "Min": ['','x'!='x'],
                                    "VDI": ''}
                            ...
                            temp_obj= {"Date_Val": dateitem,
                                        "Name":response1[ind2][ind3]['STATION_NAME'],
                                        "Max": [response1[ind2][ind3]['max_voltage'],response1[ind2][ind3]['max_voltage']==response1[ind2][ind3]['min_voltage']],
                                        "Min": [response1[ind2][ind3]['min_voltage'],response1[ind2][ind3]['max_voltage']==response1[ind2][ind3]['min_voltage']],
                                        "VDI": round(float(response1[ind2][ind3]['volt1_value'])+float(response1[ind2][ind3]['volt3_value']), 2)}
                    else:
                        if response1[ind2][ind3]['STATION_NAME']== 'Dummy':
                            temp_obj["Name"+str(ind3)]= ''
                            temp_obj["Max"+str(ind3)]= ['','x'!='x']
                            temp_obj["Min"+str(ind3)]= ['','x'!='x']
                            temp_obj["VDI"+str(ind3)]= ''
                            
                        else:
                            temp_obj["Name"+str(ind3)]= response1[ind2][ind3]['STATION_NAME']
                            temp_obj["Max"+str(ind3)]= [response1[ind2][ind3]['max_voltage'],response1[ind2][ind3]['max_voltage']==response1[ind2][ind3]['min_voltage']]
                            temp_obj["Min"+str(ind3)]= [response1[ind2][ind3]['min_voltage'],response1[ind2][ind3]['max_voltage']==response1[ind2][ind3]['min_voltage']]
                            temp_obj["VDI"+str(ind3)]= round(float(response1[ind2][ind3]['volt1_value'])+float(response1[ind2][ind3]['volt3_value']), 2)

                response1[ind2]= temp_obj

        except:
            pass

        res2 = requests.get(
            'https://report.erldc.in/posoco_api/api/PSP/GetPSPVoltageProfile_765kvByDate/'+dateitem)
        response2 = json.loads(res2.text)

        try:
            response2 = sorted(response2, key=lambda x: x['STATION_NAME'])
            response2= list(divide_chunks(response2, 3))

            dummy_dict={
                'STATION_NAME':'Dummy',
            }

            if len(response2[-1])!=3:
                if len(response2[-1])==2:
                    response2[-1].append(dummy_dict)
                else:
                    response2[-1].append(dummy_dict)
                    response2[-1].append(dummy_dict)

            for ind2 in range(len(response2)):  
                for ind3 in range(len(response2[ind2])):
                    if ind3==0:
                        if response2[ind2][ind3]['STATION_NAME']== 'Dummy':
                            temp_obj1= {"Date_Val": dateitem,
                                    "Name":'',
                                    "Max": ['','x'!='x'],
                                    "Min": ['','x'!='x'],
                                    "VDI": ''}
                            ...
                            temp_obj1= {"Date_Val": dateitem,
                                    "Name":response2[ind2][ind3]['STATION_NAME'],
                                    "Max": [response2[ind2][ind3]['max_voltage'],response2[ind2][ind3]['max_voltage']==response2[ind2][ind3]['min_voltage']],
                                    "Min": [response2[ind2][ind3]['min_voltage'],response2[ind2][ind3]['max_voltage']==response2[ind2][ind3]['min_voltage']],
                                    "VDI": round(float(response2[ind2][ind3]['volt1_value'])+float(response2[ind2][ind3]['volt3_value']), 2)}
                    else:
                        if response2[ind2][ind3]['STATION_NAME']== 'Dummy':
                            temp_obj1["Name"+str(ind3)]= ''
                            temp_obj1["Max"+str(ind3)]= ['','x'!='x']
                            temp_obj1["Min"+str(ind3)]= ['','x'!='x']
                            temp_obj1["VDI"+str(ind3)]= ''
                            
                        else:
                            temp_obj1["Name"+str(ind3)]= response2[ind2][ind3]['STATION_NAME']
                            temp_obj1["Max"+str(ind3)]= [response2[ind2][ind3]['max_voltage'],response2[ind2][ind3]['max_voltage']==response2[ind2][ind3]['min_voltage']]
                            temp_obj1["Min"+str(ind3)]= [response2[ind2][ind3]['min_voltage'],response2[ind2][ind3]['max_voltage']==response2[ind2][ind3]['min_voltage']]
                            temp_obj1["VDI"+str(ind3)]= round(float(response2[ind2][ind3]['volt1_value'])+float(response2[ind2][ind3]['volt3_value']), 2)

                response2[ind2]= temp_obj1
        ...
        if date_iter== 0:
            date_iter+= 1
            for val1 in response1:
                four_list.append([val1])
            for val2 in response2:
                seven_list.append([val2])

        else:
            for value1 in range(len(response1)):
                four_list[value1].append(response1[value1])
            for value2 in range(len(response2)):
                seven_list[value2].append(response2[value2])

    return jsonify(["Voltage", {'400KV': four_list, '765KV': seven_list}])
...
def GeneratorBreakdownReport(date_val):
    ...
    LOGBOOK_URL = "https://logbook.erldc.in:55005/api/GeneratorOutage/GetGeneratorOutageReport?date="+current_date
...
def LineTrippingReport(date_val):
    ...
    for dates in dates_list:
        TRIPPING_URL = "https://logbook.erldc.in:55005/api/HistoricElements/Tripping?month=" + \
...
def ElementBreakdownReport(date_val):
    ...
    LOGIN_URL = "https://logbook.erldc.in:55005/api/Account/Login"
    ALL_ELEMENTS_DATA_URL = "https://logbook.erldc.in:55005/api/Master/Get/AllElementsData"
    ...
    logbook_url = "https://logbook.erldc.in:55005/api/Dashboard/Get"

    mapping_url = "https://logbook.erldc.in:55005/api/Master/Get/CommonMasterData"
...
def NC_IEGC_Report(date_val):
    ...
    # print("hi", headers)
    resp = requests.get(
        "https://logbook.erldc.in:55005/api/Reports/ViolationMessage?startDate="+start_date+"&endDate="+end_date,
        headers=headers,
    )
    resp = json.loads(resp.text)
...
def dc_revision_counter():
    ...
    for dt in pd.date_range(first_day_of_month, current_datetime):
        ...
        Share_data = json.loads(Share_response)
        seller_list = []
        DC_Dict = {}
        ...
        maxRev = int(json.loads(response)["MaxRevision"])
