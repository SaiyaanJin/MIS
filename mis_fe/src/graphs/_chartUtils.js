...
export const makeGradient = (ctx, hex, height = 400) => {
	...
	gradient.addColorStop(0.5, hexRgba(hex, 0.1));
...
export const exportGraphToExcel = (data, label, prefix = "Graph") => {
	...
	const isSingleRange = Object.values(stationMap).every(
		(entries) => entries.length === 1,
	);
	const fmt = (ts) => {
		const m = moment(ts, moment.ISO_8601, true);
		return ts ? (m.isValid() ? m.format("DD-MMM-YY HH:mm") : String(ts)) : "";
	};
	const fmtT = (ts) => {
		const m = moment(ts, moment.ISO_8601, true);
		return ts ? (m.isValid() ? m.format("HH:mm") : String(ts)) : "";
	};
	...
	const hasDualBus = (entry) => {
		return (
			entry["voltageBus1"] !== undefined && entry["voltageBus2"] !== undefined
		);
	};
	...
	if (isSingleRange) {
		...
		if (hasBusData) {
			...
			const rows = sharedTimeArr.map((ts, idx) => {
				const row = { "Date / Time": fmt(ts) };
				stNames.forEach((n) => {
					row[n] = getDataValue(stationMap[n][0])[idx] ?? "";
				});
				return row;
			});
			if (rows.length > 0) {
		...
		for (let i = 0; i < data.length - 1; i++) {
			const e = data[i];
			const d = e["Date_Time"]
				? moment(e["Date_Time"]).format("DD-MMM-YYYY")
				: "Value";
	...
	saveAs(
		...
		`${prefix}_${label}_${moment().format("YYYYMMDD_HHmm")}.xlsx`,
	);
};
