...
export default function ISGSgraph(props) {
  var name = "ISGS Data of ";
  var data = [];
  var datatype = "ISGS Data";
  ...
  if (props.isgs_data) {
    ...
    for (var i = 0; i < props.isgs_data.length - 1; i++) {
      var max = props.isgs_data[i]["max"];
      var min = props.isgs_data[i]["min"];
      var avg = props.isgs_data[i]["avg"];
      ...
      if (props.date_time && props.check1) {
        name_suffix =
          ": " + moment(props.isgs_data[i]["Date_Time"]).format("DD-MM-YYYY");
        name = "Day-wise ISGS Data of ";
        datatype = "Date-wise ISGS Data ";
      }

      if (props.date_time && props.check2) {
        name_suffix =
          ": " + moment(props.isgs_data[i]["Date_Time"]).format("MMMM YYYY");

        name = " Month-wise ISGS Data of ";
        datatype = "Month-wise ISGS Data ";
      }

      if (i === 0) {
        stsn = props.isgs_data[i]["stationName"];
      } else {
        if (
          props.isgs_data[i]["stationName"] !==
          props.isgs_data[i - 1]["stationName"]
        ) {
          stsn = stsn + ", " + props.isgs_data[i]["stationName"];
        }
      }

      let isgs = {
        y: props.isgs_data[i]["output"],
        x: props.isgs_data[props.isgs_data.length - 1]["Date_Time"],
        name:
          props.isgs_data[i]["stationName"] +
          " Data" +
          name_suffix +
      ...
      if (props.freq_region1.indexOf("ISGS") > -1) {
        let V_Duration = {
          y: props.isgs_data[i]["Duration"][0],
          x: props.isgs_data[i]["Duration"][1],
          xaxis: "x2",
          name:
            props.isgs_data[i]["stationName"] +
            " Duration of " +
            moment(props.isgs_data[0]["Date_Time"]).format("DD-MM-YYYY"),
          type: "line",
        };
