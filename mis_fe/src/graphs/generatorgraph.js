...
export default function Generatorgraph(props) {
    ...
    const { chartData, chartOptions } = useMemo(() => {
        if (!props.generator_data) return { chartData: {}, chartOptions: {} };

        const datasets = [];
        const dateArr  = props.generator_data[props.generator_data.length - 1]["Date_Time"] || [];
        ...
        // ── Station traces ────────────────────────────────────────────────────
        for (let i = 0; i < props.generator_data.length - 1; i++) {
            const entry   = props.generator_data[i];
    ...
    }, [props.generator_data, props.freq_region, props.frequency, props.check1, props.check2, isDarkMode]);
    ...
    if (!props.generator_data) return null;
