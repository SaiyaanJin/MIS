...
export default function Exchangegraph(props) {
    ...
    const { chartData, chartOptions } = useMemo(() => {
        if (!props.exchange_data) return { chartData: {}, chartOptions: {} };

        const dateArr = props.exchange_data[props.exchange_data.length - 1]["Date_Time"] || [];
        const labels  = formatXLabels(dateArr);
        const datasets = [];
        let hasFreqAxis = false;

        for (let i = 0; i < props.exchange_data.length - 1; i++) {
            const entry = props.exchange_data[i];
            const max   = entry["max"];
            const min   = entry["min"];
            const avg   = entry["avg"];
    ...
    }, [props.exchange_data, props.freq_region, props.frequency, props.check1, props.check2, isDarkMode]);

    if (!props.exchange_data) return null;
