...
import ThermalGeneratorgraph from "../graphs/thermalGeneratorgraph";
...
.gen-hero {
  ...
  background: linear-gradient(135deg, #052e16 0%, #15803d 45%, #16a34a 100%);
  box-shadow: 0 12px 28px -8px rgba(22,163,74,0.45);
...
.gen-btn-primary {
  background: linear-gradient(135deg, #052e16 0%, #15803d 50%, #16a34a 100%) !important;
  border: none !important;
...
export default function ThermalGenerator() {
    ...
    const [thermalGenerator_states, setthermalGenerator_states]                         = useState();
    const [Selected_thermalGenerator_states, setSelected_thermalGenerator_states]       = useState();
    const [thermalGenerator_data, setthermalGenerator_data]                             = useState();
    const [enable, setenable]                                             = useState(true);
    const [graphenable, setgraphenable]                                   = useState(true);

    const [multiple_date, setMultiple_Date]                                                 = useState();
    const [multiple_thermalGenerator_states, setmultiplethermalGenerator_states]                         = useState();
    const [multiple_Selected_thermalGenerator_states, setmultipleSelected_thermalGenerator_states]       = useState();
    const [multiple_thermalGenerator_data, setmultiplethermalGenerator_data]                             = useState();
    ...
    useEffect(() => {
        if (start_date && end_date) {
            axios.post(baseUrl + "/ThermalGeneratorNames?startDate=" + moment(start_date).format("YYYY-MM-DD HH:mm") + "&endDate=" + moment(end_date).format("YYYY-MM-DD HH:mm"), {})
                .then(r => setthermalGenerator_states(r.data)).catch(() => {});
        }
        if (multiple_date) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            if (dates.length === multiple_date.length) {
                axios.post(baseUrl + "/MultiThermalGeneratorNames?MultistartDate=" + dates, {})
                    .then(r => setmultiplethermalGenerator_states(r.data)).catch(() => {});
            }
        }
        if (multiple_month) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            if (months.length === multiple_month.length) {
                axios.post(baseUrl + "/MultiThermalGeneratorNames?MultistartDate=" + months, {})
                    .then(r => setmultiplethermalGenerator_states(r.data)).catch(() => {});
            }
        }
    }, [start_date, end_date, multiple_date, multiple_month]);

    const getthermalGeneratordata = () => {
        if (!start_date || !end_date || !Selected_thermalGenerator_states) return;
        ...
        axios.post(baseUrl + `/GetThermalGeneratorData?startDate=${sd}&endDate=${ed}&stationName=${(Selected_thermalGenerator_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&time=${t}`, {})
            .then(r => { setthermalGenerator_data(r.data); setenable(false); setgraphenable(false); setBlocked(false); setloading_show(false); });
    ...
    const getmultithermalGeneratordata = () => {
        if (multiple_date && checked1) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (dates.length && multiple_Selected_thermalGenerator_states) {
                axios.post(baseUrl + `/GetMultiThermalGeneratorData?MultistartDate=${dates}&MultistationName=${(multiple_Selected_thermalGenerator_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Date&time=${t}`, {})
                    .then(r => { setmultiplethermalGenerator_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
        ...
        if (multiple_month && checked2) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (months.length && multiple_Selected_thermalGenerator_states) {
                axios.post(baseUrl + `/GetMultiThermalGeneratorData?MultistartDate=${months}&MultistationName=${(multiple_Selected_thermalGenerator_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Month&time=${t}`, {})
                    .then(r => { setmultiplethermalGenerator_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
    ...
    // ── Stat Badges ───────────────────────────────────────────────────────────
    const stationCount = thermalGenerator_data ? (thermalGenerator_data.length - 1) : 0;
    const multiCount   = multiple_thermalGenerator_data ? (multiple_thermalGenerator_data.length - 1) : 0;
    const dateRangeDays = start_date && end_date ? moment(end_date).diff(moment(start_date), "days") + 1 : 0;
    const selectedCount = Selected_thermalGenerator_states ? Selected_thermalGenerator_states.length : 0;
            ...
            {loading_show && (
                <div className="gen-loading-overlay">
                    <div className="gen-loading-card">
                        ...
                        <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Fetching ThermalGenerator Data</div>
                        <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Crunching numbers from the grid…</div>
                    </div>
                </div>
            )}

            <div style={{ paddingBottom: 40, '--page-accent': '#16a34a' }}>
				<div>
                ...
                <div className="grid mb-4" style={{ animationDelay: "0.05s" }}>
                    {[
                        ...
                        { color: "violet", icon: "pi-chart-line",    label: "Data Status", value: thermalGenerator_data ? "✓ Ready to view" : "Pending generation" },
                    ].map((s, idx) => (
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="ThermalGenerator Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                        ...
                        <div className="grid gap-0">
                            <div className="col-12 md:col-6">
                                ...
                                <div className="gen-overlay-group">
                                    {["ThermalGenerator", "Frequency"].map(v => (
                        ...
                        <div className="gen-action-row">
                            <Button icon="pi pi-chart-bar" label="Generate Graph" className="gen-btn-primary"
                                onClick={() => { setloading_show(true); getthermalGeneratordata(); }} />
                            <Button icon="pi pi-file-excel" label="Download Excel Data" className="gen-btn-success"
                                disabled={!thermalGenerator_data}
                                tooltip="Export plotted data as Excel" tooltipOptions={{ position: "top" }}
                                onClick={() => exportGraphToExcel(thermalGenerator_data,
                                    `${moment(start_date).format("YYYYMMDD")}_${moment(end_date).format("YYYYMMDD")}`)} />
                            
                            {!graphenable && thermalGenerator_data && (
                                <Chip label={`${thermalGenerator_data.length - 1} trace(s) loaded`}
                ...
                {!graphenable && (
                    <div className="gen-chart-wrapper mb-4">
                        ...
                        <div style={{ padding: "16px 20px" }}>
                            <ThermalGeneratorgraph
                                thermalGenerator_data={thermalGenerator_data}
                                Selected_thermalGenerator_states={Selected_thermalGenerator_states}
                                frequency={frequency}
                ...
                <div className="gen-section" style={{ animationDelay: "0.1s" }}>
                    ...
                    <div className="gen-section-body">
                        <div className="grid align-items-end gap-0">
                            ...
                            {/* ThermalGenerator multi-selector */}
                            <div className="col-12 md:col-5">
                                <div className="mb-3">
                                    <div className="gen-field-label"><i className="pi pi-bolt" />Select ThermalGenerator(s)</div>
                                    <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                        ...
                                        value={multiple_Selected_thermalGenerator_states} options={multiple_thermalGenerator_states}
                                        onChange={(e) => setmultipleSelected_thermalGenerator_states(e.value)}
                                        filter className="w-full modern-multiselect"
                                        panelClassName="modern-multiselect-panel" />
                                </div>
                            </div>
                        ...
                        <div className="gen-action-row">
                            <Button icon="pi pi-chart-bar" label="Generate Comparison" className="gen-btn-primary"
                                ...
                                onClick={() => { setloading_show(true); getmultithermalGeneratordata(); }} />
                            <Button icon="pi pi-file-excel" label="Download Excel Data" className="gen-btn-success"
                                disabled={!multiple_thermalGenerator_data}
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_thermalGenerator_data, lbl, "Thermalgenerator");
                                }} />
                            {!graphenable2 && multiple_thermalGenerator_data && (
                                <Chip label={`${multiple_thermalGenerator_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
                ...
                {!graphenable2 && (
                    <div className="gen-chart-wrapper">
                        ...
                        <div style={{ padding: "16px 20px" }}>
                            <ThermalGeneratorgraph
                                thermalGenerator_data={multiple_thermalGenerator_data}
                                Selected_thermalGenerator_states={multiple_Selected_thermalGenerator_states}
                                date_time={true}
                                check1={checked1}
                                check2={checked2}
