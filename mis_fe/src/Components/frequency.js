...
.gen-hero {
  ...
  background: linear-gradient(135deg, #7c0040 0%, #be185d 45%, #FF1493 100%);
  box-shadow: 0 12px 28px -8px rgba(255,20,147,0.45);
...
.gen-section:hover {
  ...
  box-shadow: var(--shadow-hover), 0 0 0 3px rgba(255,20,147,0.07);
}
.gen-section-header {
  padding: 18px 24px;
  display: flex; align-items: center; justify-content: space-between;
  border-bottom: 1.5px solid rgba(255,20,147,0.18);
...
.gen-section-header::before {
  ...
  width: 3px;
  background: linear-gradient(180deg, #FF1493, #be185d);
...
.gen-section-pill.blue   { background: rgba(255,20,147,0.14); color: #FF1493; border: 1px solid rgba(255,20,147,0.25); }
.gen-section-pill.violet { background: rgba(255,20,147,0.14); color: #be185d; border: 1px solid rgba(255,20,147,0.25); }
...
.gen-btn-primary {
  background: linear-gradient(135deg, #7c0040 0%, #be185d 50%, #FF1493 100%) !important;
  border: none !important;
  ...
  white-space: nowrap;
  box-shadow: 0 4px 14px rgba(255,20,147,0.4) !important;
  transition: all 0.2s !important;
}
.gen-btn-primary:hover {
  transform: translateY(-2px) !important;
  box-shadow: 0 8px 22px rgba(255,20,147,0.5) !important;
}
...
/* Download Excel — enabled (#FF1493) */
.gen-btn-secondary:not(:disabled),
.gen-btn-secondary:not([disabled]) {
  background: linear-gradient(135deg, #7c0040 0%, #be185d 60%, #FF1493 100%) !important;
  border: none !important;
  color: #fff !important;
  opacity: 1 !important;
  box-shadow: 0 4px 14px rgba(255,20,147,0.35) !important;
}
.gen-btn-secondary:not(:disabled):hover,
.gen-btn-secondary:not([disabled]):hover {
  transform: translateY(-2px) !important;
  box-shadow: 0 8px 22px rgba(255,20,147,0.45) !important;
}
.gen-btn-secondary {
  background: var(--bg-main) !important;
  border: 1px solid var(--border-bright) !important;
...
export default function Frequency() {
    ...
    const [multiple_Selected_frequency_states, setmultipleSelected_frequency_states]       = useState();
    ...
    useEffect(() => {
        if (start_date && end_date) {
            axios.post(baseUrl + "/FrequencyNames?startDate=" + moment(start_date).format("YYYY-MM-DD HH:mm") + "&endDate=" + moment(end_date).format("YYYY-MM-DD HH:mm"), {})
                .then(r => setfrequency_states(r.data)).catch(() => {});
        }
        if (multiple_date) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            if (dates.length === multiple_date.length) {
                axios.post(baseUrl + "/MultiFrequencyNames?MultistartDate=" + dates, {})
                    .then(r => setmultiplefrequency_states(r.data)).catch(() => {});
            }
        }
        if (multiple_month) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            if (months.length === multiple_month.length) {
                axios.post(baseUrl + "/MultiFrequencyNames?MultistartDate=" + months, {})
                    .then(r => setmultiplefrequency_states(r.data)).catch(() => {});
            }
        }
    ...
    const getfrequencydata = () => {
        if (!start_date || !end_date || !Selected_frequency_states) return;
        ...
        axios.post(baseUrl + `/GetFrequencyData?startDate=${sd}&endDate=${ed}&stationName=${(Selected_frequency_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&time=${t}`, {})
            .then(r => { setfrequency_data(r.data); setenable(false); setgraphenable(false); setBlocked(false); setloading_show(false); });
    ...
    const getmultifrequencydata = () => {
        if (multiple_date && checked1) {
            ...
            if (dates.length && multiple_Selected_frequency_states) {
                axios.post(baseUrl + `/GetMultiFrequencyData?MultistartDate=${dates}&MultistationName=${(multiple_Selected_frequency_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Date&time=${t}`, {})
                    .then(r => { setmultiplefrequency_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
        ...
        if (multiple_month && checked2) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (months.length && multiple_Selected_frequency_states) {
                axios.post(baseUrl + `/GetMultiFrequencyData?MultistartDate=${months}&MultistationName=${(multiple_Selected_frequency_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Month&time=${t}`, {})
                    .then(r => { setmultiplefrequency_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
    ...
    // ── Stat Badges ───────────────────────────────────────────────────────────
    const stationCount = frequency_data ? (frequency_data.length - 1) : 0;
    const multiCount   = multiple_frequency_data ? (multiple_frequency_data.length - 1) : 0;
    const dateRangeDays = start_date && end_date ? moment(end_date).diff(moment(start_date), "days") + 1 : 0;
    const selectedCount = Selected_frequency_states ? Selected_frequency_states.length : 0;
            ...
            {loading_show && (
                <div className="gen-loading-overlay">
                    <div className="gen-loading-card">
                        ...
                        <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Fetching Frequency Data</div>
                        <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Crunching numbers from the grid…</div>
                    </div>
                </div>
            )}

            <div style={{ paddingBottom: 40, '--page-accent': '#FF1493' }}>
				<div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 14, position: "relative", zIndex: 1 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <p style={{ color: "rgba(255,255,255,0.65)", margin: "3px 0 0", fontSize: 11.5, fontWeight: 400 }}>
                                Hz monitoring & stability tracking
                            </p>
                        </div>
                ...
                <div className="grid mb-4" style={{ animationDelay: "0.05s" }}>
                    {[
                        ...
                        { color: "violet", icon: "pi-chart-line",    label: "Data Status", value: frequency_data ? "✓ Ready to view" : "Pending generation" },
                    ].map((s, idx) => (
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="Frequency Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                            ...
                            <div className="col-12 md:col-5">
                                <div className="mb-3">
                                    ...
                                    <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                        ...
                                        value={multiple_Selected_frequency_states} options={multiple_frequency_states}
                        ...
                        <div className="gen-action-row">
                            <Button icon="pi pi-chart-bar" label="Generate Comparison" className="gen-btn-primary"
                                ...
                                onClick={() => { setloading_show(true); getmultifrequencydata(); }} />
                            <Button icon="pi pi-file-excel" label="Download Excel Data" className="gen-btn-success"
                                disabled={!multiple_frequency_data}
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_frequency_data, lbl, "Frequency");
                                }} />
                            {!graphenable2 && multiple_frequency_data && (
                                <Chip label={`${multiple_frequency_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
