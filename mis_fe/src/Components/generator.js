...
export default function Generator() {
    ...
    const [multiple_Selected_generator_states, setmultipleSelected_generator_states]       = useState();
    ...
    useEffect(() => {
        if (start_date && end_date) {
            axios.post(baseUrl + "/GeneratorNames?startDate=" + moment(start_date).format("YYYY-MM-DD HH:mm") + "&endDate=" + moment(end_date).format("YYYY-MM-DD HH:mm"), {})
                .then(r => setgenerator_states(r.data)).catch(() => {});
        }
        if (multiple_date) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            if (dates.length === multiple_date.length) {
                axios.post(baseUrl + "/MultiGeneratorNames?MultistartDate=" + dates, {})
                    .then(r => setmultiplegenerator_states(r.data)).catch(() => {});
            }
        }
        if (multiple_month) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            if (months.length === multiple_month.length) {
                axios.post(baseUrl + "/MultiGeneratorNames?MultistartDate=" + months, {})
                    .then(r => setmultiplegenerator_states(r.data)).catch(() => {});
            }
        }
    ...
    const getgeneratordata = () => {
        if (!start_date || !end_date || !Selected_generator_states) return;
    ...
    const exportGraphToExcel = (data, label) => {
        ...
        if (isSingleRange) {
            ...
            const rows = sharedTimeArr.map((ts, idx) => {
                ...
                stNames.forEach(n => { row[n] = (stationMap[n][0]["output"] || [])[idx] ?? ""; });
                return row;
            });
            if (rows.length > 0) {
            ...
            Object.entries(stationMap).forEach(([name, entries]) => {
                ...
                const slotCount = Math.max(...entries.map(e => (e["output"]||[]).length));
                const rows = Array.from({ length: slotCount }, (_, ri) => {
                    const row = { "Time": fmtT(sharedTimeArr[ri]) || `Slot ${ri+1}` };
                    entries.forEach((e, di) => { row[hdrs[di]] = (e["output"]||[])[ri] ?? ""; });
                    return row;
                });
                if (rows.length > 0) {
                    const ws = XLSX.utils.json_to_sheet(rows);
    ...
    // ── Stat Badges ───────────────────────────────────────────────────────────
    const stationCount = generator_data ? (generator_data.length - 1) : 0;
    const multiCount   = multiple_generator_data ? (multiple_generator_data.length - 1) : 0;
    const dateRangeDays = start_date && end_date ? moment(end_date).diff(moment(start_date), "days") + 1 : 0;
    const selectedCount = Selected_generator_states ? Selected_generator_states.length : 0;
            ...
            {loading_show && (
                <div className="gen-loading-overlay">
                    <div className="gen-loading-card">
                        ...
                        <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Fetching Generator Data</div>
            ...
            <div style={{ paddingBottom: 40 }}>
                ...
                <div className="grid mb-4" style={{ animationDelay: "0.05s" }}>
                    {[
                        ...
                        { color: "violet", icon: "pi-chart-line",    label: "Data Status", value: generator_data ? "✓ Ready to view" : "Pending generation" },
                    ].map((s, idx) => (
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="Generator Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                        <div className="grid align-items-end">
                            ...
                            <div className="col-12 md:col-5">
                                <div className="gen-field-label"><i className="pi pi-bolt" />Select Generator(s)</div>
                                <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                    ...
                                    value={Selected_generator_states} options={generator_states}
                        ...
                        <div className="gen-action-row">
                            ...
                            <Button icon="pi pi-file-excel" label="Export Raw Data" className="gen-btn-secondary"
                                ...
                                onClick={() => { window.location.href = `${baseUrl}/GetGeneratorDataExcel?startDate=${moment(start_date).format("YYYY-MM-DD")}&endDate=${moment(end_date).format("YYYY-MM-DD")}&stationName=${Selected_generator_states}`; }} />
                            {!graphenable && generator_data && (
                                <Chip label={`${generator_data.length - 1} trace(s) loaded`}
                            ...
                            <div className="col-12 md:col-5">
                                <div className="mb-3">
                                    ...
                                    <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                        ...
                                        value={multiple_Selected_generator_states} options={multiple_generator_states}
                        ...
                        <div className="gen-action-row">
                            ...
                            <Button icon="pi pi-download" label="Export Graph Data" className="gen-btn-secondary"
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_generator_data, lbl);
                                }} />
                            {!graphenable2 && multiple_generator_data && (
                                <Chip label={`${multiple_generator_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
