...
export default function Mvar() {
    ...
    const [multiple_Selected_mvar_states, setmultipleSelected_mvar_states]       = useState();
    ...
    const getmvardata = () => {
        if (!start_date || !end_date || !Selected_mvar_states) return;
        ...
        axios.post(baseUrl + `/GetMvarData?startDate=${sd}&endDate=${ed}&stationName=${(Selected_mvar_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&time=${t}`, {})
            .then(r => { setmvar_data(r.data); setenable(false); setgraphenable(false); setBlocked(false); setloading_show(false); });
    ...
    const getmultimvardata = () => {
        if (multiple_date && checked1) {
            ...
            if (dates.length && multiple_Selected_mvar_states) {
                axios.post(baseUrl + `/GetMultiMvarData?MultistartDate=${dates}&MultistationName=${(multiple_Selected_mvar_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Date&time=${t}`, {})
                    .then(r => { setmultiplemvar_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
        ...
        if (multiple_month && checked2) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (months.length && multiple_Selected_mvar_states) {
                axios.post(baseUrl + `/GetMultiMvarData?MultistartDate=${months}&MultistationName=${(multiple_Selected_mvar_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Month&time=${t}`, {})
                    .then(r => { setmultiplemvar_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
    ...
    const exportGraphToExcel = (data, label) => {
        ...
        if (isSingleRange) {
            ...
            if (rows.length > 0) {
                ...
                XLSX.utils.book_append_sheet(wb, ws, "Mvar Data");
            }
        } else {
    ...
    const selectedCount = Selected_mvar_states ? Selected_mvar_states.length : 0;
            ...
            <div style={{ paddingBottom: 40 }}>
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="Mvar Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                        ...
                        <div className="gen-action-row">
                            ...
                            <Button icon="pi pi-file-excel" label="Export Raw Data" className="gen-btn-secondary"
                                ...
                                onClick={() => { window.location.href = `${baseUrl}/GetMvarDataExcel?startDate=${moment(start_date).format("YYYY-MM-DD")}&endDate=${moment(end_date).format("YYYY-MM-DD")}&stationName=${(Selected_mvar_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}`; }} />
                            {!graphenable && mvar_data && (
                                <Chip label={`${mvar_data.length - 1} trace(s) loaded`}
                        ...
                        <div className="gen-action-row">
                            ...
                            <Button icon="pi pi-download" label="Export Graph Data" className="gen-btn-secondary"
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_mvar_data, lbl);
                                }} />
                            {!graphenable2 && multiple_mvar_data && (
                                <Chip label={`${multiple_mvar_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
