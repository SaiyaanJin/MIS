	...
	useEffect(() => {
		...
		if (multiple_date) {
			...
			if (i === multiple_date.length) {
				axios
					.post("/MultiMVARNames?MultistartDate=" + temp_multi_date, {})
					.then((response) => {
		...
		if (multiple_month) {
			...
			if (j === multiple_month.length) {
				axios
					.post("/MultiMVARNames?MultistartDate=" + temp_multi_month, {})
					.then((response) => {
						setmultiplemvar_states(response.data);
					})
					.catch((error) => {});
		...
		if (multiple_date && checked1) {
			if (multiminutes && checked4) {
				...
				if (temp1 && multiple_Selected_mvar_states) {
					axios
						.post(
							"/GetMultiMVARData?MultistartDate=" +
								temp1 +
								"&MultistationName=" +
					...
					<div className="col">
						...
						<div className="col-2">
							<a
								hidden={enable}
								href={
									`${baseUrl}/GetMVARDataExcel?startDate=` +
									moment(start_date).format("YYYY-MM-DD") +
									"&endDate=" +
									moment(end_date).format("YYYY-MM-DD") +
									"&stationName=" +
