...
.gen-hero {
  ...
  background: linear-gradient(135deg, #7c2d12 0%, #b45309 45%, #ffa500 100%);
  box-shadow: 0 12px 28px -8px rgba(255,165,0,0.45);
...
.gen-section:hover {
  ...
  box-shadow: var(--shadow-hover), 0 0 0 3px rgba(255,165,0,0.07);
}
.gen-section-header {
  padding: 18px 24px;
  display: flex; align-items: center; justify-content: space-between;
  border-bottom: 1.5px solid rgba(255,165,0,0.18);
  background: linear-gradient(135deg, rgba(255,165,0,0.06) 0%, rgba(255,165,0,0.03) 100%);
  position: relative;
}
.gen-section-header::before {
  ...
  width: 3px;
  background: linear-gradient(180deg, #ffa500, #b45309);
...
.gen-section-pill.blue   { background: rgba(255,165,0,0.14); color: #ffa500; border: 1px solid rgba(255,165,0,0.25); }
.gen-section-pill.violet { background: rgba(255,165,0,0.14); color: #b45309; border: 1px solid rgba(255,165,0,0.25); }
...
.gen-btn-primary {
  background: linear-gradient(135deg, #7c2d12 0%, #b45309 50%, #ffa500 100%) !important;
  border: none !important;
  ...
  white-space: nowrap;
  box-shadow: 0 4px 14px rgba(255,165,0,0.4) !important;
...
/* Download Excel — enabled (#ffa500) */
.gen-btn-secondary:not(:disabled),
.gen-btn-secondary:not([disabled]) {
  background: linear-gradient(135deg, #7c2d12 0%, #b45309 60%, #ffa500 100%) !important;
  border: none !important;
  color: #fff !important;
  opacity: 1 !important;
  box-shadow: 0 4px 14px rgba(255,165,0,0.35) !important;
}
.gen-btn-secondary:not(:disabled):hover,
.gen-btn-secondary:not([disabled]):hover {
  transform: translateY(-2px) !important;
  box-shadow: 0 8px 22px rgba(255,165,0,0.45) !important;
}
.gen-btn-secondary {
  background: var(--bg-main) !important;
  border: 1px solid var(--border-bright) !important;
...
export default function Demand() {
    ...
    const [multiple_Selected_demand_states, setmultipleSelected_demand_states]       = useState();
    ...
    useEffect(() => {
        if (start_date && end_date) {
            axios.post(baseUrl + "/DemandNames?startDate=" + moment(start_date).format("YYYY-MM-DD HH:mm") + "&endDate=" + moment(end_date).format("YYYY-MM-DD HH:mm"), {})
                .then(r => setdemand_states(r.data)).catch(() => {});
        }
        if (multiple_date) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            if (dates.length === multiple_date.length) {
                axios.post(baseUrl + "/MultiDemandNames?MultistartDate=" + dates, {})
                    .then(r => setmultipledemand_states(r.data)).catch(() => {});
            }
        }
        if (multiple_month) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            if (months.length === multiple_month.length) {
                axios.post(baseUrl + "/MultiDemandNames?MultistartDate=" + months, {})
                    .then(r => setmultipledemand_states(r.data)).catch(() => {});
            }
        }
    ...
    const getdemanddata = () => {
        if (!start_date || !end_date || !Selected_demand_states) return;
        ...
        axios.post(baseUrl + `/GetDemandData?startDate=${sd}&endDate=${ed}&stationName=${(Selected_demand_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&time=${t}`, {})
            .then(r => { setdemand_data(r.data); setenable(false); setgraphenable(false); setBlocked(false); setloading_show(false); });
    ...
    const getmultidemanddata = () => {
        if (multiple_date && checked1) {
            ...
            if (dates.length && multiple_Selected_demand_states) {
                axios.post(baseUrl + `/GetMultiDemandData?MultistartDate=${dates}&MultistationName=${(multiple_Selected_demand_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Date&time=${t}`, {})
                    .then(r => { setmultipledemand_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
        ...
        if (multiple_month && checked2) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (months.length && multiple_Selected_demand_states) {
                axios.post(baseUrl + `/GetMultiDemandData?MultistartDate=${months}&MultistationName=${(multiple_Selected_demand_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Month&time=${t}`, {})
                    .then(r => { setmultipledemand_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
    ...
    // ── Stat Badges ───────────────────────────────────────────────────────────
    const stationCount = demand_data ? (demand_data.length - 1) : 0;
    const multiCount   = multiple_demand_data ? (multiple_demand_data.length - 1) : 0;
    const dateRangeDays = start_date && end_date ? moment(end_date).diff(moment(start_date), "days") + 1 : 0;
    const selectedCount = Selected_demand_states ? Selected_demand_states.length : 0;
            ...
            {loading_show && (
                <div className="gen-loading-overlay">
                    <div className="gen-loading-card">
                        ...
                        <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Fetching Demand Data</div>
                        <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Crunching numbers from the grid…</div>
                    </div>
                </div>
            )}

            <div style={{ paddingBottom: 40, '--page-accent': '#ffa500' }}>
				<div>
                ...
                <div className="grid mb-4" style={{ animationDelay: "0.05s" }}>
                    {[
                        ...
                        { color: "violet", icon: "pi-chart-line",    label: "Data Status", value: demand_data ? "✓ Ready to view" : "Pending generation" },
                    ].map((s, idx) => (
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="Demand Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                        <div className="grid align-items-end">
                            ...
                            <div className="col-12 md:col-5">
                                <div className="gen-field-label"><i className="pi pi-bolt" />Select Demand(s)</div>
                                <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                    ...
                                    value={Selected_demand_states} options={demand_states}
                    ...
                    <div className="gen-section-body">
                            ...
                            <div className="col-12 md:col-5">
                                <div className="mb-3">
                                    ...
                                    <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                        ...
                                        value={multiple_Selected_demand_states} options={multiple_demand_states}
                        ...
                        <div className="gen-action-row">
                            <Button icon="pi pi-chart-bar" label="Generate Comparison" className="gen-btn-primary"
                                ...
                                onClick={() => { setloading_show(true); getmultidemanddata(); }} />
                            <Button icon="pi pi-file-excel" label="Download Excel Data" className="gen-btn-success"
                                disabled={!multiple_demand_data}
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_demand_data, lbl, "Demand");
                                }} />
                            {!graphenable2 && multiple_demand_data && (
                                <Chip label={`${multiple_demand_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
