...
.gen-hero {
  ...
  box-shadow: 0 12px 28px -8px rgba(71,85,105,0.45);
...
export default function Voltage() {
    ...
    const [multiple_Selected_voltage_states, setmultipleSelected_voltage_states]       = useState();
    ...
    useEffect(() => {
        if (start_date && end_date) {
            axios.post(baseUrl + "/VoltageNames?startDate=" + moment(start_date).format("YYYY-MM-DD HH:mm") + "&endDate=" + moment(end_date).format("YYYY-MM-DD HH:mm"), {})
                .then(r => setvoltage_states(r.data)).catch(() => {});
        }
        if (multiple_date) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            if (dates.length === multiple_date.length) {
                axios.post(baseUrl + "/MultiVoltageNames?MultistartDate=" + dates, {})
                    .then(r => setmultiplevoltage_states(r.data)).catch(() => {});
            }
        }
        if (multiple_month) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            if (months.length === multiple_month.length) {
                axios.post(baseUrl + "/MultiVoltageNames?MultistartDate=" + months, {})
                    .then(r => setmultiplevoltage_states(r.data)).catch(() => {});
            }
        }
    ...
    const getvoltagedata = () => {
        if (!start_date || !end_date || !Selected_voltage_states) return;
        ...
        axios.post(baseUrl + `/GetVoltageData?startDate=${sd}&endDate=${ed}&stationName=${(Selected_voltage_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&time=${t}`, {})
            .then(r => { setvoltage_data(r.data); setenable(false); setgraphenable(false); setBlocked(false); setloading_show(false); });
    ...
    const getmultivoltagedata = () => {
        if (multiple_date && checked1) {
            ...
            if (dates.length && multiple_Selected_voltage_states) {
                axios.post(baseUrl + `/GetMultiVoltageData?MultistartDate=${dates}&MultistationName=${(multiple_Selected_voltage_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Date&time=${t}`, {})
                    .then(r => { setmultiplevoltage_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
        ...
        if (multiple_month && checked2) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (months.length && multiple_Selected_voltage_states) {
                axios.post(baseUrl + `/GetMultiVoltageData?MultistartDate=${months}&MultistationName=${(multiple_Selected_voltage_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Month&time=${t}`, {})
                    .then(r => { setmultiplevoltage_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
    ...
    // ── Stat Badges ───────────────────────────────────────────────────────────
    const stationCount = voltage_data ? (voltage_data.length - 1) : 0;
    const multiCount   = multiple_voltage_data ? (multiple_voltage_data.length - 1) : 0;
    const dateRangeDays = start_date && end_date ? moment(end_date).diff(moment(start_date), "days") + 1 : 0;
    const selectedCount = Selected_voltage_states ? Selected_voltage_states.length : 0;
            ...
            {loading_show && (
                <div className="gen-loading-overlay">
                    <div className="gen-loading-card">
                        ...
                        <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Fetching Voltage Data</div>
                        <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Crunching numbers from the grid…</div>
                    </div>
                </div>
            )}

            <div style={{ paddingBottom: 40, '--page-accent': '#94a3b8' }}>
				<div>
                ...
                <div className="grid mb-4" style={{ animationDelay: "0.05s" }}>
                    {[
                        ...
                        { color: "violet", icon: "pi-chart-line",    label: "Data Status", value: voltage_data ? "✓ Ready to view" : "Pending generation" },
                    ].map((s, idx) => (
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="Voltage Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                        <div className="grid align-items-end">
                            ...
                            <div className="col-12 md:col-5">
                                <div className="gen-field-label"><i className="pi pi-bolt" />Select Voltage(s)</div>
                                <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                    ...
                                    value={Selected_voltage_states} options={voltage_states}
                            ...
                            <div className="col-12 md:col-5">
                                <div className="mb-3">
                                    ...
                                    <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                        ...
                                        value={multiple_Selected_voltage_states} options={multiple_voltage_states}
                        ...
                        <div className="gen-action-row">
                            ...
                            <Button icon="pi pi-file-excel" label="Download Excel Data" className="gen-btn-success"
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_voltage_data, lbl, "Voltage");
                                }} />
                            {!graphenable2 && multiple_voltage_data && (
                                <Chip label={`${multiple_voltage_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
