	...
	useEffect(() => {
		...
		if (multiple_date) {
			...
			if (i === multiple_date.length) {
				axios
					.post(
						"http://10.3.200.63:5010/MultiVoltageNames?MultistartDate=" +
							temp_multi_date,
						{}
					)
					.then((response) => {
		...
		if (multiple_date && checked1) {
			if (multiminutes && checked4) {
				...
				if (temp1 && multiple_Selected_voltage_states) {
					axios
						.post(
							"http://10.3.200.63:5010/GetMultiVoltageData?MultistartDate=" +
								temp1 +
								"&MultistationName=" +
				...
				<div className="col">
					...
					<div className="col-2">
						<a
							hidden={enable}
							href={
								"http://10.3.200.63:5010/GetVoltageDataExcel?startDate=" +
