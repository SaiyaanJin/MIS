...
.gen-hero {
  ...
  background: linear-gradient(135deg, #0c4a6e 0%, #0891b2 45%, #00d9ff 100%);
  box-shadow: 0 12px 28px -8px rgba(0,217,255,0.45);
...
.gen-section:hover {
  ...
  box-shadow: var(--shadow-hover), 0 0 0 3px rgba(0,217,255,0.07);
}
.gen-section-header {
  padding: 18px 24px;
  display: flex; align-items: center; justify-content: space-between;
  border-bottom: 1.5px solid rgba(0,217,255,0.18);
  background: linear-gradient(135deg, rgba(0,217,255,0.06) 0%, rgba(0,217,255,0.03) 100%);
  position: relative;
}
...
.gen-section-pill.blue   { background: rgba(0,217,255,0.14); color: #00d9ff; border: 1px solid rgba(0,217,255,0.25); }
.gen-section-pill.violet { background: rgba(0,217,255,0.14); color: #0891b2; border: 1px solid rgba(0,217,255,0.25); }
...
.gen-btn-primary {
  background: linear-gradient(135deg, #0c4a6e 0%, #0891b2 50%, #00d9ff 100%) !important;
  border: none !important;
...
.gen-btn-primary:hover {
  ...
  box-shadow: 0 8px 22px rgba(0,217,255,0.5) !important;
}
...
/* Download Excel — enabled (#00d9ff) */
.gen-btn-secondary:not(:disabled),
.gen-btn-secondary:not([disabled]) {
  background: linear-gradient(135deg, #0c4a6e 0%, #0891b2 60%, #00d9ff 100%) !important;
  border: none !important;
  color: #fff !important;
  opacity: 1 !important;
  box-shadow: 0 4px 14px rgba(0,217,255,0.35) !important;
}
.gen-btn-secondary:not(:disabled):hover,
.gen-btn-secondary:not([disabled]):hover {
  transform: translateY(-2px) !important;
  box-shadow: 0 8px 22px rgba(0,217,255,0.45) !important;
}
.gen-btn-secondary {
  background: var(--bg-main) !important;
  border: 1px solid var(--border-bright) !important;
...
export default function Exchange() {
    ...
    const [multiple_Selected_exchange_states, setmultipleSelected_exchange_states]       = useState();
    ...
    useEffect(() => {
        if (start_date && end_date) {
            axios.post(baseUrl + "/ExchangeNames?startDate=" + moment(start_date).format("YYYY-MM-DD HH:mm") + "&endDate=" + moment(end_date).format("YYYY-MM-DD HH:mm"), {})
                .then(r => setexchange_states(r.data)).catch(() => {});
        }
        if (multiple_date) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            if (dates.length === multiple_date.length) {
                axios.post(baseUrl + "/MultiExchangeNames?MultistartDate=" + dates, {})
                    .then(r => setmultipleexchange_states(r.data)).catch(() => {});
            }
        }
        if (multiple_month) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            if (months.length === multiple_month.length) {
                axios.post(baseUrl + "/MultiExchangeNames?MultistartDate=" + months, {})
                    .then(r => setmultipleexchange_states(r.data)).catch(() => {});
            }
        }
    ...
    const getexchangedata = () => {
        if (!start_date || !end_date || !Selected_exchange_states) return;
        ...
        axios.post(baseUrl + `/GetExchangeData?startDate=${sd}&endDate=${ed}&stationName=${(Selected_exchange_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&time=${t}`, {})
            .then(r => { setexchange_data(r.data); setenable(false); setgraphenable(false); setBlocked(false); setloading_show(false); });
    ...
    const getmultiexchangedata = () => {
        if (multiple_date && checked1) {
            ...
            if (dates.length && multiple_Selected_exchange_states) {
                axios.post(baseUrl + `/GetMultiExchangeData?MultistartDate=${dates}&MultistationName=${(multiple_Selected_exchange_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Date&time=${t}`, {})
                    .then(r => { setmultipleexchange_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
        ...
        if (multiple_month && checked2) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (months.length && multiple_Selected_exchange_states) {
                axios.post(baseUrl + `/GetMultiExchangeData?MultistartDate=${months}&MultistationName=${(multiple_Selected_exchange_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Month&time=${t}`, {})
                    .then(r => { setmultipleexchange_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
    ...
    // ── Stat Badges ───────────────────────────────────────────────────────────
    const stationCount = exchange_data ? (exchange_data.length - 1) : 0;
    const multiCount   = multiple_exchange_data ? (multiple_exchange_data.length - 1) : 0;
    const dateRangeDays = start_date && end_date ? moment(end_date).diff(moment(start_date), "days") + 1 : 0;
    const selectedCount = Selected_exchange_states ? Selected_exchange_states.length : 0;
            ...
            {loading_show && (
                <div className="gen-loading-overlay">
                    <div className="gen-loading-card">
                        ...
                        <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Fetching Exchange Data</div>
                        <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Crunching numbers from the grid…</div>
                    </div>
                </div>
            )}

            <div style={{ paddingBottom: 40, '--page-accent': '#00d9ff' }}>
				<div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 14, position: "relative", zIndex: 1 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <p style={{ color: "rgba(255,255,255,0.65)", margin: "3px 0 0", fontSize: 11.5, fontWeight: 400 }}>
                                Inter-regional exchange tracking
                            </p>
                        </div>
                ...
                <div className="grid mb-4" style={{ animationDelay: "0.05s" }}>
                    {[
                        ...
                        { color: "violet", icon: "pi-chart-line",    label: "Data Status", value: exchange_data ? "✓ Ready to view" : "Pending generation" },
                    ].map((s, idx) => (
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="Exchange Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                        ...
                        <div className="gen-action-row">
                            <Button icon="pi pi-chart-bar" label="Generate Comparison" className="gen-btn-primary"
                                ...
                                onClick={() => { setloading_show(true); getmultiexchangedata(); }} />
                            <Button icon="pi pi-file-excel" label="Download Excel Data" className="gen-btn-success"
                                disabled={!multiple_exchange_data}
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_exchange_data, lbl, "Exchange");
                                }} />
                            {!graphenable2 && multiple_exchange_data && (
                                <Chip label={`${multiple_exchange_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
