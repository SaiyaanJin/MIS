...
export default function Reports() {
	...
	const [volt_data_header_400, setvolt_data_header_400] = useState([]);
	const [volt_data_header_765, setvolt_data_header_765] = useState([]);
	const [volt_data_header_765_last, setvolt_data_header_765_last] = useState(
		[]
	);
	...
	const generate_report = (type) => {
		...
		if (date && type) {
			axios
				.post(
					"http://10.3.200.63:5010/reports?date_val=" +
						date_range +
						"&category=" +
						type,
					{}
				...
				.then((response) => {
					...
					if (response.data[0] === "Voltage") {
						setvoltage_data(response.data[1]);

						setshow_volt_table(true);
						setvolt_data_header_400(["v1", "v2", "v3", "v4", "v5"]);
						setvolt_data_header_765(["v6", "v7"]);
						setvolt_data_header_765_last(["v8"]);
	...
	// 			doc.autoTable(exportColumns, freq_data);
	// 			doc.save("Frequency_Report().pdf");
	// 		});
	// 	});
	// };
	...
	// 		const worksheet = xlsx.utils.json_to_sheet(freq_data);
			...
			<ColumnGroup>
				<Row>
					...
					<Column
						alignHeader="center"
						headerClassName="p-head1"
						header={voltage_data[e][0].Name}
						colSpan={3}
					/>
					<Column
						alignHeader="center"
						headerClassName="p-head"
						header={voltage_data[e][0].Name1}
						colSpan={3}
					/>
					<Column
						alignHeader="center"
						headerClassName="p-head1"
						header={voltage_data[e][0].Name2}
						colSpan={3}
					/>
				</Row>
				<Row>
		...
		<ColumnGroup>
			...
			<Row>
				...
				<Column
					...
					colSpan={1}
				/>
				{/* <Column alignHeader="center" headerClassName="p-head2" colSpan={1} /> */}
	...
	const volt_max = (e) => {
		...
		for (let vals of voltage_data[e]) {
			if (vals.Max[0] > max) {
				max = vals.Max[0];
			}
		}
	...
	// console.log(volt_data_header_400);
	const volt_footerGroup = (e) => {
		return (
			<ColumnGroup>
				<Row>
					<Column
				...
				<TabView>
					...
					<TabPanel header="Voltage Report" leftIcon="pi pi-sliders-v ml-2">
						...
						<div className="card" hidden={!show_volt_table}>
							...
							{volt_data_header_400.map((e) => (
								<div className="flex flex-wrap gap-1 justify-content-between align-items-center">
									...
									<div className="field">
										<DataTable
											ref={dt}
											value={voltage_data[e]}
											headerColumnGroup={volt_headerGroup(e)}
											footerColumnGroup={volt_footerGroup(e)}
											stripedRows
											showGridlines
							...
							{volt_data_header_765_last.map((e) => (
								<div className="flex flex-wrap gap-1 justify-content-between align-items-center">
									<div className="field"></div>
									<div className="field">
										<DataTable
											ref={dt}
											value={voltage_data[e]}
											headerColumnGroup={volt_headerGroup_last(e)}
											footerColumnGroup={volt_footerGroup_last(e)}
											stripedRows
											showGridlines
											cellClassName={rowClassName}
										>
