...
.gen-hero {
  ...
  background: linear-gradient(135deg, #2e1065 0%, #4c1d95 45%, #6a5acd 100%);
  box-shadow: 0 12px 28px -8px rgba(106,90,205,0.45);
...
.gen-section {
  border-radius: 16px;
  border: 1.5px solid rgba(106,90,205,0.22);
  background: var(--bg-card);
  box-shadow: var(--shadow-soft);
  overflow: hidden;
  margin-bottom: 24px;
...
.gen-section:hover {
  ...
  box-shadow: var(--shadow-hover), 0 0 0 3px rgba(106,90,205,0.07);
}
.gen-section-header {
  padding: 18px 24px;
  display: flex; align-items: center; justify-content: space-between;
  border-bottom: 1.5px solid rgba(106,90,205,0.18);
  background: linear-gradient(135deg, rgba(106,90,205,0.06) 0%, rgba(106,90,205,0.03) 100%);
  position: relative;
}
.gen-section-header::before {
  ...
  width: 3px;
  background: linear-gradient(180deg, #6a5acd, #4c1d95);
...
.gen-section-pill.blue   { background: rgba(106,90,205,0.14); color: #6a5acd; border: 1px solid rgba(106,90,205,0.25); }
.gen-section-pill.violet { background: rgba(106,90,205,0.14); color: #4c1d95; border: 1px solid rgba(106,90,205,0.25); }
...
.gen-btn-primary {
  background: linear-gradient(135deg, #2e1065 0%, #4c1d95 50%, #6a5acd 100%) !important;
  border: none !important;
  ...
  white-space: nowrap;
  box-shadow: 0 4px 14px rgba(106,90,205,0.4) !important;
  transition: all 0.2s !important;
}
.gen-btn-primary:hover {
  transform: translateY(-2px) !important;
  box-shadow: 0 8px 22px rgba(106,90,205,0.5) !important;
}
...
/* Download Excel — enabled (#6a5acd) */
.gen-btn-secondary:not(:disabled),
.gen-btn-secondary:not([disabled]) {
  background: linear-gradient(135deg, #2e1065 0%, #4c1d95 60%, #6a5acd 100%) !important;
  border: none !important;
  color: #fff !important;
  opacity: 1 !important;
  box-shadow: 0 4px 14px rgba(106,90,205,0.35) !important;
}
.gen-btn-secondary:not(:disabled):hover,
.gen-btn-secondary:not([disabled]):hover {
  transform: translateY(-2px) !important;
  box-shadow: 0 8px 22px rgba(106,90,205,0.45) !important;
}
.gen-btn-secondary {
  background: var(--bg-main) !important;
  border: 1px solid var(--border-bright) !important;
...
export default function Ict() {
    ...
    const [multiple_Selected_ict_states, setmultipleSelected_ict_states]       = useState();
    ...
    useEffect(() => {
        if (start_date && end_date) {
            axios.post(baseUrl + "/IctNames?startDate=" + moment(start_date).format("YYYY-MM-DD HH:mm") + "&endDate=" + moment(end_date).format("YYYY-MM-DD HH:mm"), {})
                .then(r => setict_states(r.data)).catch(() => {});
        }
        if (multiple_date) {
            const dates = multiple_date.map(d => moment(d).format("YYYY-MM-DD"));
            if (dates.length === multiple_date.length) {
                axios.post(baseUrl + "/MultiIctNames?MultistartDate=" + dates, {})
                    .then(r => setmultipleict_states(r.data)).catch(() => {});
            }
        }
        if (multiple_month) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            if (months.length === multiple_month.length) {
                axios.post(baseUrl + "/MultiIctNames?MultistartDate=" + months, {})
                    .then(r => setmultipleict_states(r.data)).catch(() => {});
            }
        }
    ...
    const getictdata = () => {
        if (!start_date || !end_date || !Selected_ict_states) return;
        ...
        axios.post(baseUrl + `/GetIctData?startDate=${sd}&endDate=${ed}&stationName=${(Selected_ict_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&time=${t}`, {})
            .then(r => { setict_data(r.data); setenable(false); setgraphenable(false); setBlocked(false); setloading_show(false); });
    ...
    const getmultiictdata = () => {
        if (multiple_date && checked1) {
            ...
            if (dates.length && multiple_Selected_ict_states) {
                axios.post(baseUrl + `/GetMultiIctData?MultistartDate=${dates}&MultistationName=${(multiple_Selected_ict_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Date&time=${t}`, {})
                    .then(r => { setmultipleict_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
        ...
        if (multiple_month && checked2) {
            const months = multiple_month.map(d => moment(d).format("YYYY-MM-DD"));
            const t = checked4 && multiminutes ? multiminutes : 1;
            if (months.length && multiple_Selected_ict_states) {
                axios.post(baseUrl + `/GetMultiIctData?MultistartDate=${months}&MultistationName=${(multiple_Selected_ict_states || []).map(s => String(s).replace(/&/g, "%26")).join(",")}&Type=Month&time=${t}`, {})
                    .then(r => { setmultipleict_data(r.data); setgraphenable2(false); setBlocked(false); setloading_show(false); });
    ...
    // ── Stat Badges ───────────────────────────────────────────────────────────
    const stationCount = ict_data ? (ict_data.length - 1) : 0;
    const multiCount   = multiple_ict_data ? (multiple_ict_data.length - 1) : 0;
    const dateRangeDays = start_date && end_date ? moment(end_date).diff(moment(start_date), "days") + 1 : 0;
    const selectedCount = Selected_ict_states ? Selected_ict_states.length : 0;
            ...
            {loading_show && (
                <div className="gen-loading-overlay">
                    <div className="gen-loading-card">
                        ...
                        <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Fetching Ict Data</div>
                        <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Crunching numbers from the grid…</div>
                    </div>
                </div>
            )}

            <div style={{ paddingBottom: 40, '--page-accent': '#6a5acd' }}>
				<div>
                ...
                <div className="grid mb-4" style={{ animationDelay: "0.05s" }}>
                    {[
                        ...
                        { color: "violet", icon: "pi-chart-line",    label: "Data Status", value: ict_data ? "✓ Ready to view" : "Pending generation" },
                    ].map((s, idx) => (
                ...
                <div className="gen-section">
                    <div className="gen-section-header">
                        ...
                        <Tag value="Ict Output" severity="info" rounded style={{ fontSize: 11 }} />
                    </div>
                    ...
                    <div className="gen-section-body">
                        <div className="grid align-items-end">
                            ...
                            <div className="col-12 md:col-5">
                                <div className="gen-field-label"><i className="pi pi-bolt" />Select Ict(s)</div>
                                <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                    ...
                                    value={Selected_ict_states} options={ict_states}
                        ...
                        <div className="gen-action-row">
                            ...
                            {!graphenable && ict_data && (
                                <Chip label={`${ict_data.length - 1} trace(s) loaded`}
                            ...
                            <div className="col-12 md:col-5">
                                <div className="mb-3">
                                    ...
                                    <MultiSelect filterPlaceholder="Search generators…" showSelectAll showClear resetFilterOnHide
                                        ...
                                        value={multiple_Selected_ict_states} options={multiple_ict_states}
                        ...
                        <div className="gen-action-row">
                            ...
                            <Button icon="pi pi-file-excel" label="Download Excel Data" className="gen-btn-success"
                                ...
                                onClick={() => {
                                    ...
                                    exportGraphToExcel(multiple_ict_data, lbl, "Ict");
                                }} />
                            {!graphenable2 && multiple_ict_data && (
                                <Chip label={`${multiple_ict_data.length - 1} trace(s) loaded`}
                                    icon="pi pi-check-circle"
